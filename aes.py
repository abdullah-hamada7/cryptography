SBOX = [
    
]

INV_SBOX = [
    
]

RCON = [0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]


def gmul2(x):
    return ((x << 1) ^ 0x11b) & 0xff if x & 0x80 else x << 1

def gmul(x, y):
    res = 0
    for i in range(8):
        if y & 1: res ^= x
        x = gmul2(x)
        y >>= 1
    return res


def expand_key(key):
    w = list(key)
    for i in range(16, 176, 4):
        t = w[i-4 : i]
        if i % 16 == 0:
            t = [SBOX[b] for b in (t[1:] + t[:1])]
            t[0] ^= RCON[i // 16]
        for j in range(4):
            w.append(w[i-16+j] ^ t[j])
    return w

def sub_bytes(state, inv=False):
    box = INV_SBOX if inv else SBOX
    for r in range(4):
        for c in range(4):
            state[r][c] = box[state[r][c]]

def shift_rows(state, inv=False):
    for r in range(4):
        if not inv:
            state[r] = state[r][r:] + state[r][:r]
        else:
            state[r] = state[r][4-r:] + state[r][:4-r]

def mix_cols(state, inv=False):
    for c in range(4):
        col = [state[r][c] for r in range(4)]
        if not inv:
            state[0][c] = gmul(col[0], 2) ^ gmul(col[1], 3) ^ col[2] ^ col[3]
            state[1][c] = col[0] ^ gmul(col[1], 2) ^ gmul(col[2], 3) ^ col[3]
            state[2][c] = col[0] ^ col[1] ^ gmul(col[2], 2) ^ gmul(col[3], 3)
            state[3][c] = gmul(col[0], 3) ^ col[1] ^ col[2] ^ gmul(col[3], 2)
        else:
            state[0][c] = gmul(col[0], 14) ^ gmul(col[1], 11) ^ gmul(col[2], 13) ^ gmul(col[3], 9)
            state[1][c] = gmul(col[0], 9) ^ gmul(col[1], 14) ^ gmul(col[2], 11) ^ gmul(col[3], 13)
            state[2][c] = gmul(col[0], 13) ^ gmul(col[1], 9) ^ gmul(col[2], 14) ^ gmul(col[3], 11)
            state[3][c] = gmul(col[0], 11) ^ gmul(col[1], 13) ^ gmul(col[2], 9) ^ gmul(col[3], 14)


def aes_block(data, rkeys, decrypt=False):
    state = [[data[r + c*4] for c in range(4)] for r in range(4)]
    
    if not decrypt:
        for r in range(4):
            for c in range(4): state[r][c] ^= rkeys[r + c*4]
            
        for rnd in range(1, 10):
            sub_bytes(state)
            shift_rows(state)
            mix_cols(state)
            rk = rkeys[rnd*16 : (rnd+1)*16]
            for r in range(4):
                for c in range(4): state[r][c] ^= rk[r + c*4]
        
        sub_bytes(state)
        shift_rows(state)
        for r in range(4):
            for c in range(4): state[r][c] ^= rkeys[160 + r + c*4]
    else:
        for r in range(4):
            for c in range(4): state[r][c] ^= rkeys[160 + r + c*4]
        
        for rnd in range(9, 0, -1):
            shift_rows(state, True)
            sub_bytes(state, True)
            rk = rkeys[rnd*16 : (rnd+1)*16]
            for r in range(4):
                for c in range(4): state[r][c] ^= rk[r + c*4]
            mix_cols(state, True)
            
        shift_rows(state, True)
        sub_bytes(state, True)
        for r in range(4):
            for c in range(4): state[r][c] ^= rkeys[r + c*4]
            
    return [state[r][c] for c in range(4) for r in range(4)]

def encrypt_aes(text, key):
    data = text.encode()
    pad = 16 - (len(data) % 16)
    data += bytes([pad] * pad)
    
    rkeys = expand_key(key.encode())
    res = []
    for i in range(0, len(data), 16):
        res.extend(aes_block(list(data[i:i+16]), rkeys))
    return res

def decrypt_aes(cipher, key):
    rkeys = expand_key(key.encode())
    res = []
    for i in range(0, len(cipher), 16):
        res.extend(aes_block(cipher[i:i+16], rkeys, True))
    
    pad = res[-1]
    return bytes(res[:-pad]).decode()
